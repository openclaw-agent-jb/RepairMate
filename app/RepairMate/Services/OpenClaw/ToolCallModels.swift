...
enum ToolDeclarations {
  ...
  static func allDeclarations(includeRepairMateTools: Bool = false) -> [[String: Any]] {
    var declarations = [execute]
    if includeRepairMateTools {
      declarations.append(contentsOf: RepairMateToolDeclarations.all)
    }
    return declarations
  }
