...
// MARK: - Domain Status Bar

struct DomainStatusBar: View {
  let domain: RepairDomain?

  var body: some View {
    HStack(spacing: 6) {
      Image(systemName: domain?.icon ?? "wrench.and.screwdriver")
        .font(.system(size: 10, weight: .semibold))
        .foregroundColor(accentColor)
      Text(domain?.shortLabel ?? "Auto")
        .font(.system(size: 12, weight: .medium))
        .foregroundColor(.white)
    }
    .padding(.horizontal, 12)
  ...
  private var accentColor: Color {
    switch domain {
    case .auto:        return Color(red: 0.455, green: 0.776, blue: 0.616) // mint green
    case .electronics: return .cyan
    case .appliances:  return .orange
    case .hvac:        return .blue
    case nil:          return Color(red: 0.455, green: 0.776, blue: 0.616)
    }
  }
}
