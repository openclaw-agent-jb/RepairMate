...
class OpenClawBridge: ObservableObject {
  ...
  func delegateTask(
    ...
    // Build URL with port only if it's not the default HTTPS port (443)
    let baseURL = GeminiConfig.openClawHost
    let port = GeminiConfig.openClawPort
    let urlString: String
    if port == 443 || port == 0 {
      // Standard HTTPS port or empty - don't append port
      urlString = "\(baseURL)/v1/chat/completions"
    } else {
      // Non-standard port - include it
      urlString = "\(baseURL):\(port)/v1/chat/completions"
    }
